<template>
  <div id="revenue">
    <div class="message" ref="message">
      ...
      <div class="no-messag" v-show="data.length == 0">目前没有支出数据</div>
      <el-table :data="data" :height="height" max-height="600" border v-if="data.length">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              @click.native="cancel(scope.$index, scope.row)"
              type="danger"
              size="small"
            >移除</el-button>
            <el-button
    ...
    <record :member="member" type="支出" :rowData="rowData" :upload="upload" v-show="recordShow"/>
  </div>
</template>
